...
plotsim_average <- function(tsb, met, filter_set = NULL, merge_ihw = TRUE,
  ...
  for (m in seq_along(met)){
    if(met[m]=="FDR - alpha"){
      tmp <- tsba %>% 
        filter(performanceMetric == "FDR") %>%
        dplyr::mutate(Method = gsub("-df03", "", blabel)) %>%
        dplyr::mutate(value = value - alpha)
    }else{
      tmp <- tsba %>% 
  ...
  if (clean_names) {
    ...
    if (any(grepl("adapt", ulabs))) {
      vlabs <- c(vlabs, 'adapt-glm', 'adapt-gam','adapt-gmm')
      clabs <- c(clabs, 'AdaPT (GLM)', 'AdaPT (GAM)', 'AdaPT (GMM)')
    }
  ...
  if (sum(met %in% c("FDR", "FNR", "TPR", "TNR", "FWER", "rejectprop", "FDR - alpha")) > 0){
    if (length(met)==1){
      gp <- gp +
  ...
  if (diffplot) {
    ...
    if (sum(met == "FDR - alpha")>0) {
      if (length(met)==1)
        gp <- gp +
          geom_abline(intercept = 0, slope = 0, lty = 2, color = "blue", alpha = 1/2)
    }
  }
