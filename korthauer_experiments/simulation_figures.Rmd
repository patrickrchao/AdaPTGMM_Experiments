...
gmm_results_path <- "./simulations/results/"
original_results_path <- "./simulations/data_files/"
...
resmet_file <- file.path("simulations","results-summary", "result-metrics.rds")
resmet_gmm_file <- file.path("simulations","results-summary", "gmm-result-metrics.rds")
...
gmm_resmet <- readRDS(resmet_gmm_file)
gmm_resmet[gmm_resmet$Method == "AdaPTGMM",]$Method <- "adapt-gmm" 
resmet <- rbind(resmet,gmm_resmet)
saveRDS(resmet,"./simulations/results-summary/full-result-metrics.rds")
```
...
objects <- list.files(original_results_path, recursive=TRUE, pattern="rds", full.names=TRUE )
resfile <- objects[grepl("yeast-", objects) & 
                   grepl("de5", objects) & 
                  !grepl("uninf", objects)] 

objects_gmm <- list.files( gmm_results_path, recursive=TRUE, pattern="rds", full.names=TRUE )
resfile_gmm <- objects_gmm[grepl("yeast-", objects_gmm) & 
                   grepl("de5", objects_gmm) & 
                  !grepl("uninf", objects_gmm)] 
de5 <- readRDS(file=resfile)
de5_gmm <- readRDS(file=resfile_gmm)

res5d <- plotsim_standardize(rbind(de5,de5_gmm), alpha = seq(0.01, 0.10, 0.01)) 
res5d$blabel[is.na(res5d$blabel) | res5d$blabel == "AdaPTGMM"] <- "adapt-gmm"
res5d <- res5d %>% filter(blabel %in% methodset)
p2a <- plotsim_average(res5d, met="FDR",
      ...
      scale_y_continuous("FDR", breaks=seq(0, 1, by=0.02)) +
      expand_limits(y=0.11, x = 0.105) +
      theme(plot.title = element_text(face = "bold", size = 12)) 


p2e <- plotsim_average(res5d, met="FDR - alpha",
                merge_ihw = TRUE, errorBars=TRUE) +
      ggtitle("Resampling with spike-ins") +
      scale_x_continuous(expression(paste(bold(alpha), bold(" level"))),
                         breaks=seq(0, 1, by=0.02)) +
      scale_y_continuous(expression(paste(bold("FDR - "),bold(alpha), bold(" level"))),  breaks=seq(-1, 1, by=0.01)) +
      # expand_limits(y=0.11, x = 0.105) +
      expand_limits(y=c(-0.06,0.01), x = 0.105) +
      theme(plot.title = element_text(face = "bold", size = 12)) 

## Figure 2C: FDR of methods in representative polyester setting at varying alpha levels
objects <- list.files(original_results_path, recursive=TRUE, pattern="rds", full.names=TRUE )
objects_gmm <- list.files(gmm_results_path, recursive=TRUE, pattern="rds", full.names=TRUE )
...
resfile_gmm <- objects_gmm[grepl("polyester-", objects_gmm) & 
                   grepl("de5", objects_gmm) & 
                  !grepl("uninf", objects_gmm)] 
...
res5dp <- plotsim_standardize(rbind(de5,de5_gmm), alpha = seq(0.01, 0.10, 0.01)) 
res5dp$blabel[is.na(res5dp$blabel)] <- "adapt-gmm"
res5dp <- res5dp%>% filter(blabel %in% methodset)
...
p2f <- plotsim_average(res5dp, met="FDR - alpha",
                merge_ihw = TRUE, errorBars=TRUE) +
      ggtitle("Polyester count simulation") +
      scale_x_continuous(expression(paste(bold(alpha), bold(" level"))),
                         breaks=seq(0, 1, by=0.02)) +
...
mid <- plot_grid(p2e + theme(legend.position="none"), p2f, nrow = 1)


bot <- plot_grid(p2c, p2d, nrow = 1)
Fig2 <- plot_grid(p2abTitle, top, mid, p2cdTitle, bot, nrow=5, 
                  rel_heights = c(0.1, 1, 1, 0.1, 1),
                  labels = c("A", "", "B", "C",""))
Fig2 <- plot_grid(Fig2, get_legend(p2a), rel_widths = c(1, .2))
Fig2
ggsave(file.path(outdir, "Figure2.pdf"), width=7.5, height=9.5)
```
...
bot <- plot_grid(p2c, p2d, nrow = 1)
Fig2 <- plot_grid(p2abTitle, mid, p2cdTitle, bot, nrow=5, 
                  rel_heights = c(0.1,  1, 0.1, 1),
                  labels = c("A", "", "B", ""))
Fig2 <- plot_grid(Fig2, get_legend(p2a), rel_widths = c(1, .2))
Fig2
ggsave(file.path(outdir, "Figure2_no_fdr.pdf"), width=7.5, height=7)
```
...
datsub <- res5d %>% filter(performanceMetric == "TPR",
                                        abs(alpha-0.10) < 1e-6,
                                        blabel %in% c("adapt-glm", "lfdr", "fdrreg-t","adapt-gmm")) %>%
                                 group_by(blabel, alpha) %>%
                                 summarize(value = mean(value)) %>%
                                 mutate(Method = blabel) %>%
                                 arrange(desc(Method))
p3a <- plotsim_average(res5d, met="TPR",
      ...
      geom_label_repel(data=datsub,
                 aes(alpha, value, label = Method),
                 min.segment.length = unit(0, 'lines'),
                 nudge_x = c(10,10,-0.01,-0.02), size = 2.25, nudge_y=c(0.008,0.008, 0.008,0.00), #l,f,gmm,glm
                 fill = "white", alpha = 1,
...
datsub <- res5dp %>% filter(performanceMetric == "TPR",
                                        abs(alpha-0.10) < 1e-6,
                                        blabel %in% c("lfdr", "fdrreg-t","adapt-gmm")) %>%
                                 group_by(blabel, alpha) %>%
                                 summarize(value = mean(value)) %>%
                                 mutate(Method = blabel) 
...
p3b <- plotsim_average(res5dp, met="TPR",
      ...
      geom_label_repel(data=datsub,
                 aes(alpha, value, label = Method),
                 min.segment.length = unit(0, 'lines'),
                 nudge_x = c(-0.02,10,10), nudge_y = c(0.01,0.01,-0.002), # g,f,l
                 size = 2.25, 
                 box.padding = 0.1, label.padding = 0.1,
                 fill = "white", alpha = 1, show.legend = FALSE)
...
legend <- get_legend(p3b)
top <- plot_grid(p3a + theme(legend.position="none"), p3b+ theme(legend.position="none"), nrow = 1)
